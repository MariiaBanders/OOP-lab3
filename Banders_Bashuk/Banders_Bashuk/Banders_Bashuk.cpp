...
public:
	...
	static inline float sum()
	{
		return 1 - ((((LnArccosExp<N, I>::sum()) * (LnArccosExp<N, I>::sum())) * (2 * K - 3)) / (2 * K - 1)) * ArctgLnArccosExpSeries <N * go, I * go, (K + 1) * go, L * go>::sum();
	}
};
...
public:
	...
	static inline float sum()
	{
		return 1 - (((ArctgLnArccosExp<N, I>::sum()*ArctgLnArccosExp<N, I>::sum()) / (2 * K - 2)) / (2 * K - 1)) * SinArctgLnArccosExpSeries <N * go, I * go, (K + 1) * go, L * go>::sum();
	}
};
...
public:
	...
	static inline float sum()
	{
		return ArctgLnArccosExp<N, I>::sum() * SinArctgLnArccosExpSeries <N, I, 2, k>::sum();
		//return float(N) / I;
	}
};
